...
def test_voice_file_success(voice_files: dict[str, str]) -> None:
    model = WhisperModels.WHISPER1
    voice = Voice(model)

    result = voice.get_text_from_audio(voice_files["mp3"])

    assert result == "Hello"


@mock.patch("openai.Audio.transcribe", transcribe_mock_result)
def test_voice_file_fail(voice_files: dict[str, str]) -> None:
    model = WhisperModels.WHISPER1
    voice = Voice(model)

    with pytest.raises(WhisperException):
        voice.get_text_from_audio(voice_files["mp5"])

    with pytest.raises(WhisperException):
        voice.get_text_from_audio(voice_files["no_file"])

    with pytest.raises(WhisperException):
        voice.get_text_from_audio(voice_files["big_size_mp4"])
